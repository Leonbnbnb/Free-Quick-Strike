...
- `js/game.js`：全部游戏逻辑与渲染（单文件）。**文件顶部有一份「模块边界」清单（数据 DATA / 逻辑 LOGIC / 渲染 RENDER / UI VIEW），加东西时按层归位**；另外「渲染层只读世界、且不消耗 world / combat 随机流」是可验证的不变量（回归台有断言），改渲染时别破坏它。关键区块：
  ...
  - `netConnect()` / `netSend()` / `netOn()` / `netClose()` / `netAbort()` / `netHostBegin()` / `netGuestBegin()` / `netGuestTick()` / `netSnapshot()` / `netApplySnap()` / `netSendInput()` / `netStartSnapshots()` / `netFetchInfo()` / `netJoinRef` / `NET_SNAP_MS`（约 18Hz）：**联机内核（V1.37）**（Supabase Realtime **public broadcast** 频道 `room:<房间码>`，手写 Phoenix 裸 WebSocket；**房主跑完整模拟、客机只上行输入 + 回放快照**；`netConnect` **必须等 `phx_join` 确认后才 resolve**、早发广播会被服务端**静默丢弃**；**join 的回执必须按 `netJoinRef`（本次 join 真正的 ref）认，不能写死 `'1'`** —— `netRef` 全局自增且 `netClose()` 不清零，写死会让第 2 次起的连接卡满 9 秒超时才失败、还误报成「联机通道未响应」；开局只发种子不发世界）
  ...
  - `netFxQueue` / `netSendFx()` / `netGuestFx()`：**打击反馈同步（V1.37）**（房主在 `spawnDamageNumber` / `spawnFloatText` 里顺手攒队列，随快照节拍额外发一条 `fx { shake, f }`（一次 ≤ `NET_FX_MAX` 条，**纯表现、可丢、不重发**）；客机把条目摆进自己的 `damageNumbers` / `shake` —— 这两个本来就是渲染层自行推进的数据。顿帧 `hitStop` 与音效仍未同步）
