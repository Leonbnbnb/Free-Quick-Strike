...
- `js/game.js`：全部游戏逻辑与渲染（单文件）。**文件顶部有一份「模块边界」清单（数据 DATA / 逻辑 LOGIC / 渲染 RENDER / UI VIEW），加东西时按层归位**；另外「渲染层只读世界、且不消耗 world / combat 随机流」是可验证的不变量（回归台有断言），改渲染时别破坏它。关键区块：
  ...
  - `updatePlayerCombat(p, dt)` / `updatePlayerCompanions(p, dt)` / `stampNewBullets(p, from)` / `joyVec()`：**双相位 + 归属戳**（`update()` 里两相位**刻意夹住 `updatePendingLightning`** 以保持单人执行顺序不变；`stampNewBullets` 按数组长度差给「本段新产生的」子弹打 `owner`；键盘与摇杆的移动意图合并在 `joyVec()`，`updateSquad` 与联机上行共用）
