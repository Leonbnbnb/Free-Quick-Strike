...
- `js/game.js`：全部游戏逻辑与渲染（单文件）。**文件顶部有一份「模块边界」清单（数据 DATA / 逻辑 LOGIC / 渲染 RENDER / UI VIEW），加东西时按层归位**；另外「渲染层只读世界、且不消耗 world / combat 随机流」是可验证的不变量（回归台有断言），改渲染时别破坏它。关键区块：
  ...
  - `makePlayer(id)` / `players[]` / `P1` / `activePlayer` / `captureCtx(p)` / `loadCtx(p)` / `switchTo(p)` / `withCtx(p, fn)` / `playerOf(s)` / `soldiersOf(p)` / `squadOf(p)` / `hpOf(p)`：**玩家槽位（V1.37 双人）**。`squad / soldiers / squadHp / squadMaxHp / stats / weapons / summons / pet / skills / pickCount / routePicks / appliedIds / dmgBonus / dmgBase` + **`lightningCdT / lightningPending / hurtBy / lastHurt / petRunExp`**（后 5 个原本是全局单份，双人下会互相干扰）这批全局**始终代表「当前生效的玩家」**，所以现有 40 多个战斗函数一行没改；**单人局 `switchTo(P1)` 是空操作**（零开销、零行为变化）。**`soldier.owner` 存玩家 id（数字），不存玩家对象引用** —— 引用会让 `soldiers ⇄ player` 成环，`saveRun()` 的 `JSON.stringify` 抛错后被 try/catch 吞掉、`meta.run` 静默置空（「返回主菜单后继续」失效）
  - `creditPlayer(id, dmg, killed)` / `playerStats()` / `player.dmgDealt` / `player.killsCount` / `killEnemy(e, byId)` / `e.burnBy` / `e.bleedBy` / `renderGameOverPanel` 里的 `#go-versus`：**按玩家拆的战绩（V1.37）**（结算页给每人一行「伤害 / 击杀」，单人局一行「本局」、双人局分「你 / 队友」）。**记账入口只有 `creditPlayer()`**：直击 / 召唤物 / 宠物 / 子弹记给「当前生效玩家」（它们都在 `withCtx` 里跑）；**DoT（点燃 / 割裂）在世界相位按 dt 结算、那时没有上下文**，所以由 `applyBurn` / `applyBleed` 当场把施加者记在敌人身上（`e.burnBy` / `e.bleedBy`），结算时显式传给 `killEnemy(e, byId)`；落雷直击 / 处决 / 冻伤这几处不走 `hitEnemy`，各自在伤害处补一笔。**这两个累计值直接写槽位、不进全局镜像**（只增不减，跨玩家记账不需要切换上下文）；「我」是哪条由各端用 `localPlayer().id` 自己认（房主 0 / 客机 1），所以 `run:end` 不用带「谁是房主」）
  - `nearestSoldier()` / `forEachSoldierAll(fn)` / `anySoldierIn(x, y, r)`：**跨玩家判定**（半径必须用**那名玩家自己的** `bodyR()`，所以内部逐个 `withCtx` 取；7 处范围伤害与站位分离都走它）
  - `COOP_HP` / `COOP_SPAWN` / `coopExtra()` / `coopHpMul()` / `coopSpawnMul()`：**双人难度放大**（V1.37：小怪与首领血量 ×1.6、出怪节奏 ×1.25，相乘正好 **2.0 倍** —— 按实测「同等装备下两人火力 = 单人的 2.00 倍」定的；**单人局恒为 1，零行为变化**。伤害刻意不动。血量在 `spawnEnemy()` 里乘，出怪在 `spawnInterval()` 的 `pace` 上乘；**新加「按人数放大」的东西都从这两处进**）
  - `updatePlayerCombat(p, dt)` / `updatePlayerCompanions(p, dt)` / `stampNewBullets(p, from)` / `joyVec()`：**双相位 + 归属戳**（`update()` 里两相位**刻意夹住 `updatePendingLightning`** 以保持单人执行顺序不变；`stampNewBullets` 按数组长度差给「本段新产生的」子弹打 `owner` —— **它只盖这两个相位**，所以在 `updateBullets` 里由命中触发的子弹（火球 `fireFireballBullet`、散弹分裂碎片 `tryShotgunSplit`）必须**自己带 `owner: idOfActive()`**，否则 `updateBullets` 的 `players[b.owner]` 会拿到 `players[undefined]` 而按「当前生效玩家」结算；键盘与摇杆的移动意图合并在 `joyVec()`，`updateSquad` 与联机上行共用）
  - `updatePendingLightning(dt)`：**延迟落雷 + 按玩家的窗口账本**（V1.37：`lightningCdT` / `lightningPending` 在槽位里，而它跑在**两个玩家相位之间的世界相位**、没有上下文 —— 窗口计时必须**逐个玩家 `withCtx` 推进**，否则只有当前生效玩家的窗口在走，客机第一发之后就再也劈不出来）
  - `beginLevelUp()` / `localPlayer()` / `rollPickFor(p)` / `setLocalCards(cards)` / `openLocalPick()` / `renderPickWaiting()` / `resolvePick(p, id)` / `finishUpgrade()` / `applyUpgradeCard(card)` / `applyUpgrade(id)` / `netGuestPick(p)`：**升级 · 各自选卡（V1.37）**（等级与经验共享、**候选各抽各的**；**世界在所有人都选完之前冻结**；**面板 `upgrades` 与待选槽位 `player.pendingPick` 必须是同一条数组引用**，否则重掷后会按旧 id 找不到卡；**待选为空一律置 `null`** —— 空数组是真值，会让「等所有人选完」永远判不成立）
  - `netConnect()` / `netSend()` / `netOn()` / `netClose()` / `netAbort()` / `netHostBegin()` / `netGuestBegin()` / `netGuestTick(dt)` / `netSnapshot()` / `netApplySnap(dt)` / `applySnapList()` / `netSendInput()` / `netStartSnapshots()` / `netFetchInfo()` / `netJoinRef` / `NET_SNAP_MS`（约 18Hz）/ `NET_SNAP_JUMP`：**联机内核（V1.37）**（Supabase Realtime **public broadcast** 频道 `room:<房间码>`，手写 Phoenix 裸 WebSocket；**房主跑完整模拟、客机只上行输入 + 回放快照**；`netConnect` **必须等 `phx_join` 确认后才 resolve**、早发广播会被服务端**静默丢弃**；**join 的回执必须按 `netJoinRef`（本次 join 真正的 ref）认，不能写死 `'1'`** —— `netRef` 全局自增且 `netClose()` 不清零，写死会让第 2 次起的连接卡满 9 秒超时才失败、还误报成「联机通道未响应」；开局只发种子不发世界）。**`netGuestTick(dt)` 只管三件事：贴快照 + 推进表现层寿命 + 算镜头** —— 客机不跑 `update()`，所以「**只由 `update()` 推进的东西**」它必须自己补（`updateDamageNumbers(dt)` 与 `shake` 衰减），漏了就是「伤害数字永不消失 + 越积越多 → 卡 + 画面一直抖」（V1.37 修）。**`netApplySnap` 必须复用同一批实体对象**（每帧重建上百个敌人 / 子弹会让 GC 频繁介入，这是客机发卡的另一个主因）、位置按帧长平滑逼近（`k = Math.min(1, dt*18)`，与刷新率无关）、**旧实体离新目标超 `NET_SNAP_JUMP`（200px）直接吸附**（数组中间少一个会让后面整体错位一格，吸附只闪一下、不会整排滑过去）
  - `renderGameOverPanel(res)` / `netBackToLobby(keepResult)` / `netLeaveMatch()` / `netOnEnd(p)`：**联机收场链路（V1.37）**（房主 `gameOver()` 广播 `run:end { won, wave, kills, level, time, coins, cause, peerCause }` → 客机摆结算页；客机不跑模拟、也没有「学到的机制」统计，所以金币那行写「由房主那一侧结算」，`fromNet: true` 就是这两个差异的开关）。**通道的存活周期是「在房间里」而不是「这一局」** —— 一局结束后两边都还在房间，房主点「开始双人对战」即可再打一局；`run:end { left: true }` = 中途收场。退回大厅必须 `players = [P1]` + `P1.id = 0` + 清 `pendingPick`，否则残留的双人槽位会被后面的单人局带进去
  - `netFxQueue` / `netSendFx()` / `netGuestFx()` / `netSfxQueue` / `netSfxTap()` / `SFX_BY_KEY` / `NET_FX_MAX` / `NET_SFX_MAX` / `triggerHitStop(d)` / `netHitStopPulse`：**打击反馈同步（V1.37）**（房主在 `spawnDamageNumber` / `spawnFloatText` 里顺手攒队列，随快照节拍额外发一条 `fx { shake, f, snd, stop }`（一次 ≤ `NET_FX_MAX` 条反馈 + ≤ `NET_SFX_MAX` 条音效，**纯表现、可丢、不重发**）；客机把数字 / 浮动文字摆进自己的 `damageNumbers`、`shake` 取较大值、照 `snd` 重放音效、把 `stop` 接到 `hitStop`。**打击音效在「事件层」记**（`netSfxTap` 放在各 `sfx*` 函数的 `audioReady` / `sfxReady` 之前）—— 客机听不听得到由客机自己的音效设置决定，节流客机自己会做一遍；**顿帧必须走 `triggerHitStop()`**（不要直接给 `hitStop` 赋值，否则客机收不到），且下发的是**脉冲**而不是剩余时长 —— 顿帧 0.02s 短于 55ms 的快照节拍，读剩余时长只能撞上约 1/3。这些**不参与任何判定**，但**寿命推进写在 `update()` 尾部**，客机得自己在 `netGuestTick()` 里补（见上））
  ...
  - `FRIEND_LISTS` / `friendData` / `FRIENDS_POLL_MS` / `friendsApi()` / `applyFriends()` / `loadFriends()` / `friendAction()` / `addFriendFromInput()` / `renderFriends()` / `renderFriendBadge()` / `renderMeAvatar()` / `avatarCanvasOf()` / `startFriendsPolling()`：**好友系统**（V1.34 立，V1.35 入口改到设置齿轮正下方、V1.35 加聊天；红点徽章 = 收到申请条数 **+ 收到的房间邀请条数**；好友表**刻意不进 Realtime 发布**，用 8~10 秒轮询兜底；好友列表渲染一律传 `defaultCharacter()`，因为**还原不出对方的角色配色**）
  - `COOP_CODE_ALPHA` / `COOP_POLL_MS` / `coopRoom` / `coopInvites` / `normCoopCode()` / `coopCodeOk()` / `roomsApi()` / `coopCreate()` / `coopJoin()` / `coopLeave()` / `coopToggleReady()` / `coopCopyInvite()` / `coopInvite()` / `coopDecline()` / `renderCoop()` / `coopSlotEl()` / `coopInviteEl()` / `loadCoopInvites()` / `refreshCoop()` / `startCoopPolling()` / `coopHandleUrl()`：**合作房间**（V1.36 只做「房间系统」：建房 / 邀请 / 加入 / 双方就位；**局内双人同步尚未实现**，双方都点「准备」也只是大厅状态位）。房间码 = **6 位、字母表去掉 `I/O/0/1`**，前端只做「去空白 + 大写」再校验；**一个账号同时只在一个房间里**（服务端在建房 / 进房前先腾席位）；`?room=<码>` 的邀请链接**整次会话只处理一次**；**房间状态一律以服务端为准**，前端只渲染、不自造状态
...
- `api/rooms.js`：线上合作房间接口（`GET /api/rooms?code=` 取房间快照、`?invites=1` 取收到的邀请；`POST {action, code, ...}`，action ∈ `create/join/ready/invite/leave/decline`；身份取自令牌）。`server.js` 的 `handleRoomsApi` 是本地等价实现（落盘 `data/rooms.json` + `data/room-invites.json`），契约一致；建表见 `supabase/migrations/0007_rooms.sql`（`rooms` + `room_invites`，RLS 零策略、不进 Realtime）。**改契约要 DB / api / server.js / game.js / html+css / docs 六层一起改**，并同步 `docs/部署流程.md` 的验证清单
