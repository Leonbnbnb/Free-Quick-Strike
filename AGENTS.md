...
- `js/game.js`：全部游戏逻辑与渲染（单文件）。**文件顶部有一份「模块边界」清单（数据 DATA / 逻辑 LOGIC / 渲染 RENDER / UI VIEW），加东西时按层归位**；另外「渲染层只读世界、且不消耗 world / combat 随机流」是可验证的不变量（回归台有断言），改渲染时别破坏它。关键区块：
  ...
  - `netConnect()` / `netSend()` / `netOn()` / `netClose()` / `netAbort()` / `netHostBegin()` / `netGuestBegin()` / `netGuestTick()` / `netSnapshot()` / `netApplySnap()` / `netSendInput()` / `netStartSnapshots()` / `netFetchInfo()` / `NET_SNAP_MS`（约 18Hz）：**联机内核（V1.37）**（Supabase Realtime **public broadcast** 频道 `room:<房间码>`，手写 Phoenix 裸 WebSocket；**房主跑完整模拟、客机只上行输入 + 回放快照**；`netConnect` **必须等 `phx_join` 确认后才 resolve**、早发广播会被服务端**静默丢弃**；开局只发种子不发世界）
